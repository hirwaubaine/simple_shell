...
/**
 * get_builtin - Matches a command with a corresponding shellby builtin func
...
int shellby_cd(char **args, char __attribute__((__unused__)) **front)
{
	...
	if (args[0])
	{
		if (*(args[0]) == '-' || _strcmp(args[0], "--") == 0)
		{
			if ((args[0][1] == '-' && args[0][2] == '\0') ||
			    args[0][1] == '\0')
			{
				if (_getenviron("OLDPWD") != NULL)
					(chdir(*_getenviron("OLDPWD") + 7));
			}
			else
			{
				free(oldpwd);
	...
	{
		if (_getenviron("HOME") != NULL)
				chdir(*(_getenviron("HOME")) + 5);
	}
